...
void main() {
  ...
  setUp(() async {
    ...
    final collection = Collection(id: 'collection', name: 'Test', createdAt: DateTime.utc(2026),
      embeddingProfile: const EmbeddingProfile(id: 'test'));
    final chat = Chat(id: 'chat', collectionId: collection.id, title: 'Test',
      createdAt: DateTime.utc(2026), updatedAt: DateTime.utc(2026), messages: []);
    container = ProviderContainer(overrides: [
      localStorageProvider.overrideWithValue(storage),
      settingsProvider.overrideWith((ref) => SettingsNotifier(storage, const AppSettings(openRouterApiKey: 'test'))),
      currentCollectionProvider.overrideWith((ref) => collection),
      currentChatProvider.overrideWith((ref) => chat),
      paperRepositoryProvider(collection.id).overrideWith((ref) => _Repository(storage)),
    ]);
  });
  tearDown(() async { container.dispose(); await directory.delete(recursive: true); });

  test('Double-send and stop while saving cannot start a paid request', () async {
  ...
  test('Stop saves the partial answer once and clears streaming state', () async {
    final bytes = StreamController<List<int>>();
    final requested = Completer<void>();
    final client = MockClient.streaming((request, body) async {
      await body.drain<void>();
      requested.complete();
      return http.StreamedResponse(bytes.stream, 200, headers: {'content-type': 'text/event-stream'});
    });
    await http.runWithClient(() async {
      ...
      final subscription = container.listen(chatControllerProvider, (_, state) {
        if (state.streamingText == 'Partial answer' && !rendered.isCompleted) rendered.complete();
      });
      bytes.add(utf8.encode('data: ${jsonEncode({
      ...
      expect(saved!.messages.map((message) => message.role), ['user', 'assistant']);
...
class _Repository extends PaperRepository {
  _Repository(LocalStorage storage) : super(storage: storage, collectionId: 'collection');
  @override
