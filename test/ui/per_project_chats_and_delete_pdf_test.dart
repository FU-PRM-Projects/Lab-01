...
void main() {
  testWidgets('Sidebar displays per-project chat sections and does not show Recents', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            projectChatsProvider(col1.id).overrideWith(
              (ref) => ChatsNotifier(storage, col1.id, [chat1]),
            ),
            projectChatsProvider(col2.id).overrideWith(
              (ref) => ChatsNotifier(storage, col2.id, [chat2]),
            ),
          ],
          child: MaterialApp(
            home: Scaffold(
  ...
  testWidgets('ArtifactPanel shows Delete PDF button and confirmation dialog', (tester) async {
